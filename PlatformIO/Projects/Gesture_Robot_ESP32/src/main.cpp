...
// --- Định nghĩa chân Motor Trái (1) - Phản hồi chậm hơn ---
#define ENCA_L 34  
#define ENCB_L 35  
#define ENA_L  14  
#define IN1_L  26  
...
// --- Định nghĩa chân Motor Phải (2) - Phản hồi nhanh hơn ---
#define ENCA_R 18  
#define ENCB_R 19  
#define ENB_R  15  
#define IN3_R  4   
...
// Thông số cơ khí
const float L_TRACK = 0.20;   // m (Khoảng cách 2 bánh)
const float R_WHEEL = 0.035;  // m (Bán kính bánh xe)

// --- Biến PID Trái (Tăng hệ số để kích bốc cho động cơ yếu) ---
...
// --- Biến PID Phải (Giữ hệ số ổn định cho động cơ khỏe) ---
...
// --- Giải mã động học nghịch (Inverse Kinematics) ---
...
{  
  ...
  // Tính vận tốc tuyến tính từng bánh từ lệnh của ROS 2
  float v_L = v - (w * L_TRACK / 2.0);
  float v_R = v + (w * L_TRACK / 2.0);

  // Quy đổi vận tốc m/s sang tốc độ vòng/phút (RPM) mục tiêu
...
// --- Trình phục vụ ngắt đọc Encoder ---
...
void setup() {
  // Mở cổng báo cáo tình hình kết nối lên máy tính nếu cần debug
  Serial.begin(115200);
  ...
  // Cấu hình linh kiện cho hệ thống bên Trái
  ...
  // Cấu hình linh kiện cho hệ thống bên Phải
  ...
  // Lắng nghe dữ liệu điều khiển từ topic /cmd_vel của Python
...
void loop() {
  ...
  // Chu kỳ tính toán PID siêu tốc 30ms
  if (currentTime - previousMillis >= 30) {
    ...
    // Tính vận tốc RPM thực tế từ số xung encoder thu được
    currentRPM_L = ((float)count_L / PPR) / (deltaT) * 60.0;
    currentRPM_R = ((float)count_R / PPR) / (deltaT) * 60.0;

    // --- TÍNH TOÁN PID MÔ-TƠ TRÁI ---
    float error_L = targetRPM_L - currentRPM_L;
    eIntegral_L += (error_L * deltaT);
    if (eIntegral_L > 500) eIntegral_L = 500; else if (eIntegral_L < -500) eIntegral_L = -500;
    if (targetRPM_L == 0) eIntegral_L = 0;
    ...
    // --- TÍNH TOÁN PID MÔ-TƠ PHẢI ---
    float error_R = targetRPM_R - currentRPM_R;
    eIntegral_R += (error_R * deltaT);
    if (eIntegral_R > 500) eIntegral_R = 500; else if (eIntegral_R < -500) eIntegral_R = -500;
    if (targetRPM_R == 0) eIntegral_R = 0;
